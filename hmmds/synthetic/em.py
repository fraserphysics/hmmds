...
def main(argv=None):
    ...
    for i in range(em_iterations):
        ...
        for t in range(T):
            p0t = (alpha[i]/numpy.sqrt(2*numpy.pi)) * numpy.exp((-(Y[t]-mu_i[i][0])**2)/2)
            p1t = ((1-alpha[i])/numpy.sqrt(2*numpy.pi)) * numpy.exp((-(Y[t]-mu_i[i][1])**2)/2)
    ...
    import pickle
    pickle.dump({'Y':Y, 'alpha':alpha, 'mu_i':mu_i}, open(args.out_path,'wb'),
                protocol=2)
