...
def main(argv=None):
    ...
    # FixMe: Why doesn't locals() work?
    data_dict = pickle.load(open(args.data, 'rb'))
    _global = globals()
    _global.update(data_dict)  # Puts x_fine, y_fine, x_coarse,
    # y_coarse, means,
    # and covariances in global namespace
    ...
    axis_x_01_short.plot(list(range(len(x_fine)))[::10],
                         x_fine[::10, 0],
                         marker='.',
                         markersize=8,
                         linestyle='None')
    axis_y_short.plot(y_fine)
    axis_y_short.plot(list(range(len(y_fine)))[::10],
                      y_fine[::10],
                      marker='.',
                      markersize=8,
                      linestyle='None',
    ...
    axis_x0_long.plot(means[:, 0], label='filtered')
    # Error of filter estimate and calculated variance of filter
    axis_error.plot(x_coarse[:, 0] - means[:, 0], label='error')
    sigma = numpy.sqrt(covariances[:, 0, 0])
