...
class BundleNode(anytree.NodeMixin):
    ...
    def __init__(
        ...
        # Calling code should assign score_t_item
        self.score_t_item = None
...
class HMM(hmm.base.HMM):
    ...
    def bundle_decode(self: HMM,
        ...
        # Leaves sorted by score + fudge * t
        leaves_by_score_t = sortedcontainers.SortedList()
        # Sorted scores of leaves
        leaves_by_score = sortedcontainers.SortedList()

        def remove_from_lists(node: BundleNode):
            """Remove entries in the lists leaves_by_score_t and leaves_by_score
            ...
            """
            score_t_item = node.score_t_item
            score_item = score_t_item.reference
            leaves_by_score_t.remove(score_t_item)
        ...
        def propagate(node):
            ...
            for b_child in range(n_bundles):
                ...
                # score = log(prob(y_0^t|b_0^t) + c_t
                p_state = p_state_and_y * bundle_and_state[b_child, :]
                norm = p_state.sum()
                if norm == 0.0:
                    continue  # b_child is not a possible successor of node
                score = float(node.score + numpy.log(norm))

                # Assign results to data structures
                child = BundleNode(b_child, score, p_state / norm, node, None)
                score_t_item = ItemScoreT(score + fudge * t, child,
                                          score)  # ToDo: score/t?
                child.score_t_item = score_t_item  # Enable removal
                leaves_by_score_t.add(score_t_item)
        ...
        while best_end is None or leaves_by_score[0] < best_end.score:
            ...
            if n_leaves % 1_000 == 0:
                print(
                    'n_leaves= {0} depth= {1} n_times={5} score_t= {2:7.4f} range(score_t) = {3:7.4f} {4:7.4f}'
                    .format(n_leaves, node.depth, leaf_score.number,
                            leaves_by_score_t[0].number,
                            leaves_by_score_t[-1].number, n_times))
            ...
            for child in children:
                ...
                if best_end is None or child.score > best_end.score:
                    print(
                        ...
                        .format(len(leaves_by_score_t), child.score,
...
def main(argv=None):
    ...
    nodes_by_score_t = sortedcontainers.SortedList()
    nodes_by_score_t.add(ItemScoreT(.1, 1))
    nodes_by_score_t.add(ItemScoreT(.1, 2))
    nodes_by_score_t.add(ItemScoreT(.2, 1))
    nodes_by_score_t.add(ItemScoreT(0, 1))
    print('nodes_by_score range from {0} to {1}'.format(
        nodes_by_score_t[0].number, nodes_by_score_t[-1].number))
    return 0
