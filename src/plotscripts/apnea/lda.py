...
def parse_args(argv):
    ...
    parser.add_argument('--apnea_data_dir',
                        type=str,
    ...
    parser.add_argument('--lda_name',
                        type=str,
                        default='lda_data',
                        help='Name of file that has lda data')
...
def main(argv=None):
    ...
    frequencies = 120 * numpy.arange(
        length_raw) / length_raw  # FixMe: Explain 120
    figure1, (ax_means, ax_components) = pyplot.subplots(nrows=2,
                                                         ncols=1,
                                                         figsize=(5, 5.5),
                                                         sharex=True)

    # n_limit = int(length_raw*.3)
    n_limit = int(length_raw * .9)

    # FixMe: Figure suggests that normalization is off.  Perhaps OK
    # because norm(mean of normalized vectors) is <= 1.
    def limited_plot(ax, x, y, n_limit, color, label):
        ax.plot(x[:n_limit], y[:n_limit], color, label=label)

    # Plot mean psds for each class
    ...
    for i, color, label in zip(
        ...
        (0, 1),
            ...
            # Label
        ('1', '2')):
        limited_plot(ax_components, frequencies, lda_dict['basis'][i], n_limit,
                     color, f'$v_{label}$')
