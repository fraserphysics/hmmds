...
$(LASER_DATA)/%.plot_data: $(LASER_CODE)/optimize_ekf.py $(LASER_DATA)/%.parameters
	python $< --method skip --plot_data $@ $(LASER_DATA)/$*.parameters

$(LASER_DATA)/pf_%.plot_data: $(LASER_CODE)/optimize_particle.py $(LASER_DATA)/%.parameters
	python $< --method skip --plot_data $@ $(LASER_DATA)/$*.parameters

$(LASER_DATA)/gui.plot_data: $(LASER_CODE)/optimize_ekf.py $(LASER_DATA)/explore.txt
	python $< --method skip --plot_data $@ --parameter_type GUI_out $(LASER_DATA)/explore.txt

$(LASER_DATA)/pf_ekf250.plot_data: $(LASER_CODE)/optimize_particle.py $(POWELL250)
	python $< --method skip --plot_data $@ $(POWELL250)

$(LASER_DATA)/pf_opt_noise.plot_data: $(LASER_CODE)/optimize_particle.py $(POWELL250) $(LASER_DATA)/pf_powell500.parameters
	python $< --method skip --plot_data $@ --override_parameters $(LASER_DATA)/pf_powell500.parameters $(POWELL250)
...
$(LASER_DATA)/pf_hand_noise.plot_data: $(LASER_CODE)/optimize_particle.py $(POWELL250) pf_hand_noise.parameters
	python $< --method skip --plot_data $@ --override_parameters pf_hand_noise.parameters $(POWELL250)

$(LASER_DATA)/%.particle_data: $(LASER_CODE)/optimize_particle.py $(LASER_DATA)/%.parameters
	python $< --method skip --n_particles 300 --plot_data $@ $(LASER_DATA)/$*.parameters

$(LASER_DATA)/pf_powell500.parameters: $(LASER_CODE)/optimize_particle.py $(POWELL250)
	python $< --length 175 --n_particles 500 --method Powell $(POWELL250) $@
...
$(LASER_DATA)/particle.parameters: $(LASER_CODE)/optimize_particle.py $(POWELL250)
	python $< --length 175 --n_particles 100 --method Powell $(POWELL250) $@
...
$(POWELL250): $(LASER_CODE)/optimize_ekf.py explore.txt
	python $< --parameter_type GUI_out --laser_data $(LP5DAT) --length 250 --method Powell explore.txt $@
