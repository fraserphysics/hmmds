...
class Fit:
    ...
    def __init__(self, names: list, a_model_record: dict, c_model_record: dict,
                 low, high, levels):
        ...
        Args:
            ...
            c_model_record: Eg, c_model_record["a01"].model was fit to c records
    ...
    def threshold(self, a_model_record, c_model_record):
        ...
        a[1] = a_model_record.log_likelihood
        a[2] = c_model_record.log_likelihood
        a[3] = 1
...
def main(argv=None):
    ...
    fit = Fit(args.record_names, a_model_record, c_model_record, args.low,
              args.high, args.levels)

    statistics = {}
    for name in args.all_names:
        pass1 = a_model_record[name].pass1
        a_log_likelihood = a_model_record[name].log_likelihood
        c_log_likelihood = c_model_record[name].log_likelihood
        fit_threshold = fit.threshold(a_model_record[name],
                                      c_model_record[name])
        if name[0] == 'x':
            best_threshold = 1.0
        else:
        ...
        statistics[name] = numpy.array([
            pass1, a_log_likelihood, c_log_likelihood, 1.0, fit_threshold,
            best_threshold
        ])
