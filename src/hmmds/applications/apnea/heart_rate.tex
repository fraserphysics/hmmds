...
\subsection{Multi-Chain Models}
\label{sec:multichain}

Multiple chains for apnea heart rate signals without normalization.

\StudyFigs{multichain}{with \textbf{different chains} for different prominences
  of \textbf{unnormalized} heart rate signals}


\subsection{Normalized Multi-Chain Models}
\label{sec:normchains}

\StudyFigs{normchain}{with \textbf{different chains} for different prominences
  of \textbf{normalized} heart rate signals}
...
choose include:
\begin{description}
\item[Sample frequencies] For VARG (Vector AutoRegressive Gaussian)
  model of the combined respiration -- low-pass filtered heart rate
  signal and for the peak detector.
\item[Filter parameters] For the low-pass filtered heart rate and for
  deriving the respiration signal.
\item[AR order] 
\item[$\psi$ and $\nu$] Priors for each state
\item[Prominence threshold] 
...
\begin{enumerate}
\item Write new utilities.read\_slow\_peaks\_intervals function that
  computes intervals with resolution that is independent of
  args.heart\_rate\_sample\_frequency.  It should report slow and peaks at
  the heart\_rate\_sample\_frequency.
\item \label{parameter_dicts} Replace make\_observation with dicts as
  arguments to make\_observation\_model functions.
\item Make a two\_varg\_masked model.
\item Modify other model types to use \ref{parameter_dicts}
\end{enumerate}
