...
Chose minimum prominence based on Fig.~\ref{fig:prominence_study}.
...
\begin{figure}
  \centering
    ...
    \caption{Error rate on the \emph{a} records for models with
      ...
      \emph{two\_ar3\_intervals*} .}
  \label{fig:prominence_study}
\end{figure}

Chose minimum the exponent for weighting components of the observation
and the detection threshold based on the study illustrated in
Fig.~\ref{fig:exponent_threshold}.

\begin{figure}
  \centering
    \resizebox{0.8\textwidth}{!}{\includegraphics{exponent_threshold.pdf}}
    \caption{Error rate of model vs the weighting exponent and the
      detection threshold with model \emph{two\_ar4\_masked} on the
      \emph{a} records.}
  \label{fig:exponent_threshold}
\end{figure}
...
To do:
\begin{itemize}
\item Plot performance against threshold for peaks (prominence\_study.py)
\item Plot performance against both exponent for interval component of
  ...
  setting the threshold too low. (test\_score2.py)
\item Plot performance vs AR order (compare\_models.py)
\item Try scaling heart rate by high frequency power.
\item Look at both performance on all records and fixable records.
\item Vary the number of peak sizes
\item Vary the minimum peak prominence
\end{itemize}
