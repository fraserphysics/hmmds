...
from a peak detector to initally supervise the training of the seed
model.
...
\begin{description}
\item[Good Overall] Record a20 is which is at the bottom of both
  ...
  most of the time, but it would be even better if it used the outlier
  state for noisy patches rather than using the slow states.  Using
  the slow states there yields unreasonably low heart rates.
...
\item[x29] At 98.625\%, the x29 model also scores badly for
  plausibity.  The leads are not connected for the first 18 minutes.
