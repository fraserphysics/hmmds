...
apnea.  \marginpar{FixMe: HR in bpm}
\begin{figure}
...
\begin{figure}
  ...
  {Information about respiration in high frequency bands of power
    spectra.  This is the $a11$ record roughly between minutes 20 and
    150.  The upper plot is heart rate (bandpass filtered 0.09-3.66
    bpm), the middle plot is a spectrogram of the heart rate, and the
    lower plot is the expert classification.  A single band of
    spectral power between about 10 and 20 bpm without much power
    below the band in the spectrogram indicates normal respiration.}
  \label{fig:sgram}
\end{figure}
...
\marginpar{FixMe: Label PQRST. (t-56minutes)/seconds}
\begin{figure}
...
each record.
\marginpar{FixMe: sharex. label axes}
\begin{figure}
...
\marginpar{FixMe: (time - 5:52)/second}
\begin{figure}
...
\marginpar{FixMe: t/minutes, a03 Raw Heart Rate/bpm}
\begin{figure}
...
\marginpar{FixMe: Get values from code}
\begin{description}
\item[Autoregressive Order] 8
\item[Model Sample Frequency] 4 samples per minute
\item[Low Pass Period] 65 seconds
\item[Respiration Center Frequency] 12 bpm
\item[Respiration Filter Width] 3.6 bpm
\item[Respiration Smoothing] 0.47 bpm
\end{description}
