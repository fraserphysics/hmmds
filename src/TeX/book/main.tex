...
%%%
%%% \includeonly{introduction, algorithms, variants, continuous, toys, apnea, appendix}
%%% \includeonly{introduction, algorithms, variants, continuous, toys, appendix}
%%%
...
%%% I'm guessing that the paper size given to 'crop' might change for
%%% actual printing runs.  The logical paper size is determined by the
%%% newsiambook.cls where it arbitrarily sets \pageheight 10in
%%% \pagewidth 7in (line 1121), regardless of any \documentclass
%%% options such as 'letterpaper' or 'a4'.  (Those will set it
%%% initially, but by the time the .cls file is done loaded, they are
%%% set to those hard-coded values.)
%%%
%%% The effective default options are (AFAICT):
%%%
%%% plain-opener,10pt,twoside,onecolumn,openright,final
%%%
%%% It doesn't seem to honor or implement 'twoside' at all.  I'm not
%%% sure what happens at the the publisher with that.  They must have
%%% a style that they load that sets it up for the press run or
%%% something.  The way it is not, it's not really set for two sided
%%% printing the way I would expect.  It looks like every page is a
%%% right-page to me.  It's too bad that the Development and
%%% Aquisitions Editor would not provide me with a direct-contact
%%% email address to a LaTeX Compositor, because it would be useful
%%% and valuable for me to be able to communicate with that expert.
%%%
\documentclass[dvips]{hmmdsbook}
...
%%% For MathTime fonts:
%%%
%%% Does not work for me: \usepackage{siammathtime}
%%% Does not work either: \usepackage{mathtime}
%%%                       \usepackage{mathpi}
%%%
%%% It bombs where there's a call to a macro for a bold-math font.  A
%%% test document typeset with 'mathtime' works... I'm not sure what's
%%% happening.  I think that either the Postscript viewer or 'dvips'
%%% is performing a font substitution, since I'm certain that the
%%% actual MathTime fonts are not installed on my computer.  There are
%%% TeX font metrics for them, but no actual fonts.  From what I can
%%% tell, they are slightly smaller than CM glyphs, so the labels on
%%% the illustrations should come out fine, and probably won't cause
%%% any over-typing from boxes being bigger than with CM.
%%%
%%%
%%% Note: SIAM Compositor: Please see the definition of and comment
%%% Note:                  regarding the '\plotsize' macro, which you
%%% Note:                  will find in "hmmdsbook.cls".
%%% Note:
%%% Note:                  Also, please grep the document for
%%% Note:                  occurances of "\longpage", and see if when
%%% Note:                  you run it with MathTime fonts whether
%%% Note:                  those pages need to be squashed or not.
%%% Note:
%%% Note:                  There is some trouble around sec:gaussian
%%% Note:                  where, in our runs, just under
%%% Note:                  fig:ScalarGaussian there is a paragraph
%%% Note:                  fragment; some awkward whitespace.  I put
%%% Note:                  a commented off \newpage above the heading
%%% Note:                  "Reestimation" that I think will fix it,
%%% Note:                  but you need to run it with MathTime first
%%% Note:                  of course.
%%%
...
\ReviewerNote{Reviewer4 wants emphasis on applications and discussion of
  characteristics that suggest using HMMs in preface and
  introduction.}
...
\ReviewerNote{
  ...
  \begin{itemize}
  \item Reviewer 1 suggests using Bayes nets for time series.
  \item Reviewer 4 says that I \emph{should} discuss these things
    briefly and have references.  I agree.  Perhaps put it at the end of
    the book so that I can describe the ideas in terms of HMMs that I
...
\ReviewerNote{Reviewer4 wants bibliography structured into basic books,
  comprehensive books, etc.}
...
\begin{CVSID}
%%%$Id: main.tex,v 1.69 2009/05/22 22:27:59 amfsr Exp $
\end{CVSID}
\end{document}
%%%---------------
%%% Local Variables:
