...
\begin{align}
  ...
  \label{eq:QoutG3}
  &= - \sum_{s\in\states} \sum_{t=0}^{T-1} w(s,t) \left( \frac{1}{2}
...
\begin{align}
  \label{eq:NewMuG}
  \mu_s &= \sum_{t=0}^{T-1} w(s,t) \ti{y}{t} \\
  \sigma_s^2 &= \sum_{t=0}^{T-1} w(s,t)\left( \ti{y}{t}-\mu_s \right)^2
\end{align}
...
\begin{figure}[htbp]
  ...
  \label{fig:MLEfail}
\end{figure}
\marginpar{ToDo: Get text inside circles of fig:MLEfail}
...
\begin{equation}
  ...
  Q_{\text{observation}} (\parameters',\parameters) =  \frac{1}{2}
      \sum_{s\in\states} \sum_{t=0}^{T-1} w(s,t) \left[ \log\left( \left|
...
can write code that maximizes $ \sum_{t=0}^{T-1} w(s,t) \left[ \log \left|
...
\item \label{step:cov} Calculate a new covariance matrix\footnote{One
  ...
  Since the value of $A_s$ that minimizes $ \sum_{t=0}^{T-1} w(s,t) -
...
\ref{step:cov}. }
  \begin{equation}
    ...
    \Sigma_s = \frac{\sum_{t=0}^{T-1} w(s,t) z_s(t)
      z_s\transpose(t)}{\sum_{t=0}^{T-1} w(s,t)}
  \end{equation}
\end{enumerate}
...
\begin{multline*}
  ...
  Q_{\text{observation}} = C + \sum_s \left( \frac{\alpha}{2}\log\left|
    ...
    \Sigma_s^{-1}) \right) \\
  + \frac{1}{2} \sum_s \sum_{t=0}^{T-1} w(s,t) \left[ \log \left|
...
\begin{equation}\label{eq:NewSigma}
  \Sigma_s = \frac{\beta \id + \sum_{t=0}^{T-1} w(s,t) z_s(t)
    z_s\transpose(t)} {\alpha + \sum_{t=0}^{T-1} w(s,t)}.
\end{equation}
...
Figure~\ref{fig:VARGstates} illustrates the strength \ToDo{Edit this
  paragraph or the figure if the figure doesn't support it.} of the
