...
A dynamical system model uses a rule, $F$, to map states $x\in\cal X$
forward in time.  Familiar examples include discrete time maps
%% \begin{equation*}
...
often think of sequences of observations as arising from applying a
measurement function, $G$, to sequences of states of a dynamical system with
$ \ti{y}{t} = G(\ti{x}{t})$.  The function $F$ that maps states
forward in time and the function $G$ that maps states to observations
make up a \emph{\index*{state space model}} for sequences of
observations.  If the observation function is not invertible, then
knowledge of the observation $\ti{y}{t}$ at a single time $t$ is not
sufficient to specify the state $\ti{x}{t}$ uniquely, and one could
say that $\ti{x}{t}$ is \emph{hidden}.  That is the sense of
\emph{hidden} in the term ``hidden Markov model''.
...
$F\circ F(x) = \hat x$, then the observations are sufficient to
...
For most of the state space models that we consider, the function,
$F$, that governs the state dynamics and the measurement function,
$G$, both have random elements.  Only imagination limits what
constitutes the set of states in a state space model.  The kinds of
observations are similarly varied.  We will consider discrete state
spaces that are sets with a finite number of elements and state spaces
that are real vector spaces.  As a prelude, we look at a sequence of
observations of a laser system that is ``Lorenz like''.
...
Eqn.~\eqref{eq:Lorenz}, $x = (x_0,x_1,x_2)$ is a vector of mode
...
1992 %
\index{Tang, D.\ Y.}%
Tang \etal\cite{Tang92,Tang94}\ reported observations of the time %
dependent intensity of the electric field for such a laser.  In their
model the observations are measurements of a component of the state,
\ie, $\ti{y}{t} = G(\ti{x}{t}) = (\ti{x_0}{t})^2$.
...
(\ti{x_0}{t})^2 + O_g$ where $S_g$ and $O_g$ are fixed scale and
...
approximate probabilities which we write as
$P_{*\given \theta_{\text{EKF}}}$ where $\theta_{\text{EKF}}$ denotes
the collection of parameters.  By passing that routine and Tang's data
to the SciPy optimization package, we found a parameter vector that
satisfies
\begin{equation}
  \label{eq:theta-hat-laser}
  \hat \theta_{\text{EKF}} = \argmax_{\theta_{\text{EKF}}} ~ P(\ts{y}{0}{250}\given \theta_{\text{EKF}}),
\end{equation}
where $P(\ts{y}{0}{250}\given \theta_{\text{EKF}})$, the \emph{likelihood} of
$\theta_{\text{EKF}}$, is the conditional probability that a sequence of 250
observations will have the values
$\ti{y}{0},\ti{y}{1}, \ldots,\ti{y}{249}$ given the parameters
$\theta_{\text{EKF}}$.  The parameter vector $\hat \theta_{\text{EKF}}$ is called the
...
\begin{figure}[htbp]
  ...
  \caption[\comment{fig:LaserLogLike }Log likelihood as function of
  $s$ and $b$.]%
  {An illustration of how the log-likelihood,
    $\log \left(P(\ts{y}{0}{250}\given \theta_{\text{EKF}}) \right)$, depends on
    the components $s$ and $b$ of $\theta_{\text{EKF}}$.  The other nine
    components of $\theta_{\text{EKF}}$ were held fixed at the values that maximize
    the likelihood.  Descriptions of all eleven parameters appear on
    page~\pageref{page:eleven_parameters}.}
  \label{fig:LaserLogLike}
\end{figure}

Given $\hat \theta_{\text{EKF}}$, the maximum likelihood parameters, and the
...
\begin{equation}
  ...
  P(\ts{x}{0}{250}\given \ts{y}{0}{250},\hat \theta_{\text{EKF}}),
\end{equation}
...
\begin{equation}
  \label{eq:xhat250}
  \hat x = \argmax_{\ti{x}{250}} P(\ti{x}{250}\given \ts{y}{0}{250},\hat \theta_{\text{EKF}}).
\end{equation}
...
\begin{figure}[htbp]
  ...
  {State trajectory $\ts{\hat x}{0}{250}$ estimated from
    ...
    Eqn.~\eqref{eq:xhatSeq}).  Components $x_0$ and $x_2$ of the
...
suppose only that a conditional probability distribution\footnote{The
  upper case subscript $X$ in $P_{\ti{X}{t+1}\given \ti{X}{t}}$
  denotes a \emph{random variable}.  See the Notation section that
  starts on page \pageref{sec:notation}.}
$P_{\ti{X}{t+1}\given \ti{X}{t}}$ governs evolution in state space and
another conditional distribution $P_{\ti{Y}{t}\given \ti{X}{t}}$
...
the \emph{\index*{stochastic process}} or \emph{information source}
\index{source!information} consisting of sequences of observations.
We refer to such a combination as a \emph{model} and denote it as
$P_{*\given \theta}$.  So defined, the class of state space models is
so broad that to do anything useful, we must use smaller subclasses.
Typically, we assume that the conditional distributions are time
invariant and that a finite set of parameters $\theta$ specifies the
model.  Notice that we have not specified the sets from which we draw
the states or observations; they could be discrete, real scalars, real
vectors, or something else.
...
\item[Model Parameter Estimation] Given a model class $\left\{
  ...
  \end{equation}
  to characterize the information source $Y$.
...
$\ti{X}{1}$, and $\ti{X}{2}$ constitute a \emph{\index*{Markov chain}}
if
\begin{equation}
...
\emph{Markov process} \index{Markov process} if for any integer
$t: 1 < t < T-1$ the variables before and after $t$ are conditionally
independent given $\ti{X}{t}$, \ie,
\begin{equation}
...
sequence of states $\ts{s}{0}{T}$, which we write\footnote{We use
  ...
  mean.} as $P_{\ts{S}{0}{T}}\left( \ts{s}{0}{T} \right)$.

For example we calculate the probability that a sequence of 4 states
has the values $\ts{s}{0}{4} = (u,v,w,v)$, (\ie, $\ti{s}{0} = u$,
$\ti{s}{1} = v$, $\ti{s}{2} = w$, and $\ti{s}{3} = v$) using the
general formula
\begin{align}
  \label{eq:intro_hmm1}
  P(\ts{s}{0}{4}) &= P(\ti{s}{0}) \prod_{t=1}^3 %
                     P(\ti{s}{t}\given \ts{s}{0}{t-1})\\
  \label{eq:intro_hmm2}
                  &= P(\ti{s}{0}) \prod_{t=1}^3 %
                    P(\ti{s}{t}\given \ti{s}{t-1}),
\end{align}
with the specified values to obtain
\begin{align}
  \label{eq:intro_hmm3}
...
\end{align}
Applying \index*{Bayes rule} $\left( P_{A\given B} P_B = P_{A,B} \right)$
...
distribution, \ie, given a transition matrix $M$ find the probability
...
\begin{equation*}
  P_{\ti{S}{1}}  = P_{\ti{S}{0}}M = P_{\ti{S}{0}} = P_{\ti{S}{t}}
  \forall t,
\end{equation*}
...
\begin{equation*}
  \lim_{t \rightarrow \infty} P_{\ti{S}{t}} =  \lim_{t \rightarrow
    \infty} P_{\ti{S}{0}} M^t = \begin{bmatrix} \frac{1}{7}, & \frac{4}{7}, & \frac{2}{7}
\end{bmatrix}.
\end{equation*}
...
produced a sequence of integer valued observations
$\ts{y}{0}{\syntheticNLorenzBig}$ $y_1^{\syntheticNLorenzBig}$ by
binning $x_0$ with boundaries at
$\syntheticboundaryA,\syntheticboundaryB$ and $\syntheticboundaryC$.
The result is that for each integer $0\leq t < \syntheticNLorenzBig$,
$\ti{x_0}{t\cdot0.15}$ yields $\ti{y}{t} \in \left\{0,1,2,3\right\}$.
...
\begin{figure}[htbp]
  ...
  {Generating the observations $\ts{y}{0}{40}$.  The curve in the
    upper plot depicts the first component $x_0(\tau)$ of an orbit of
    the Lorenz system (Eqn.~\ref{eq:Lorenz}), and the points marked
    with red dots indicate the values sampled with an interval
    $\Delta \tau = 0.15$.  The points in the lower plot are the
    $\ti{y}{t}$ values which are $\ti{x_0}{t}$ values quantized with
    boundaries
    $\left\{\syntheticboundaryA, \syntheticboundaryB,
      \syntheticboundaryC\right\}$.}
  \label{fig:TSintro} 
\end{figure}
...
the Viterbi algorithm to find the most likely state sequence
 ...
 \begin{figure}[p]
   \centering{\plotsize%
     \includegraphics[width=0.95\textwidth]{Statesintro}}%
 ...
 \begin{figure}[p]
   \centering{\plotsize%
     \includegraphics[width=1.0\textwidth]{GraphStates}}%
   \caption[\comment{fig:GraphStates }Graph of transitions between
   states.]%
...
\begin{table}[htb]
  ...
  \begin{center}{\footnotesize%
      ...
      \begin{tabular}[t]{r@{\hspace{0.28em}}p{19em}}
        1  -- & \rule{0pt}{2.5ex}Nominative pronouns \\
