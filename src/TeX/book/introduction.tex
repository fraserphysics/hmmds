...
\begin{figure}[htbp]
  \centering{\plotsize%		% See the comment at the definition
    \def\t{$t$}%		% of \plotsize for why it exists and
    \def\yt{$\ti{y}{t}$}%	% why these labels are macros.
    \def\Laser{Laser}%
    \def\Simulation{Simulation}%
    \input{figs/LaserLP5.eps_t}}
...
\end{equation}
%%% \ToDo{ Note that I need smoothing to do this right.}

\begin{figure}[htbp]
  \centering{\plotsize%
    \input{figs/LaserStates.eps_t}}
...
\begin{figure}[htbp]
  \centering{\plotsize%
    \def\tiyt{$\ti{y}{t}$}%
    \def\Lase{Laser}%
    \def\Forecas{Forecast}%
    \input{figs/LaserForecast.eps_t}}
  \caption[\comment{fig:LaserForecast }Forecast observation sequence.]%
...
\ReviewerNote{Reviewer 1 wants a discussion of Kalman vs HMM and
  suggests citing Roweis.}
\ReviewerNote{ Reviewer 2 wants:
  \begin{itemize}
  \item A more leisurely introduction
  \item He also wants exercises, and so does Patrick Campbell
  \end{itemize}
}
...
discrete in time, state, and observation.  \ReviewerNote{Reviewer 2
  wants more on unhidden Markov models.}  We begin with a couple of
definitions.  Three random variables $\ti{X}{1}$, $\ti{X}{2}$, and
$\ti{X}{3}$ constitute a \emph{\index*{Markov chain}} if
\begin{equation}
...
\ReviewerNote{Reviewer 2 thinks that this is about transients not
  about a sequence.  I should expand until it's clear.  Probably
  mention \emph{relaxation to stationary measure}.  $\ldots$ which
  should perhaps be defined in a ``nomenclature'' entry.}
...
state is relevant. %
%%%
\ReviewerNote{Reviewer 2 wants an example in section
  \ref{sec:intro_hmm} with a more concrete interpretation of states
  and transition probabilities.  It might be fun to use Markov's
  original example.  PC says if I do that, I should put it at the end
  of the section.}
...
\begin{figure}[htbp]
  \centering{\plotsize%
    \input{figs/Markov_dhmm_net.eps_t}}
...
\begin{figure}[htbp]
  \centering{\plotsize%
    \input{figs/TSintro.eps_t}}
  \caption[\comment{fig:TSintro }Generating the observations $\ts{y}{1}{40}$.]%
...
%%%
 \begin{figure}[p]
   \centering{\plotsize%
     \includegraphics[width=1.0\textwidth]{figs/Statesintro}}%
...
\ReviewerNote{
  \begin{itemize}
  \item Reviewer 2 suggests a transition diagram
  \item Reviewer 2 wants suggested applications that justify the
    effort
  \item Review 3 wants connections between HMMs and other approaches
  \end{itemize}
}
...
\longpage%% FixMe: Maybe not when set with MathTime fonts
...
\begin{CVSID}
%%%$Id: introduction.tex,v 1.85 2007-06-07 05:12:02 karlheg Exp $
\end{CVSID}
