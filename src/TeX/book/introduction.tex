...
\item[Classification] Given sample signals like $\ts{y}{0}{T}$ and two
  possible signal sources, $\alpha$ and $\beta$ where $P_{*\given \alpha}$
  characterizes healthy units and $P_{*\given \beta}$ characterizes defective
  units, one can can classify a unit on the basis of the
  \emph{likelihood ratio}
  \begin{equation*}
    R(\ts{y}{0}{T}) = \frac{P(\ts{y}{0}{T}\given \beta)}{P(\ts{y}{0}{T}\given \alpha)}.
  \end{equation*}
...
\end{description}
The results appear in Table~\ref{tab:POS}.\ToDo{Perhaps drop
  punctuation from table.}
