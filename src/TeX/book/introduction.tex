...
measured quantity corresponds to $(\ti{x_1}{t})^2$. \ToDo{This is G above}
...
$\theta$ denotes the collection of parameters.  \ToDo{Diane confused
  about theta.  Maybe say 11 dim.  Make clear that this theta is not
  an HMM theta.} By passing that
...
\ToDo{Footnote about notation for probability}
...
\item[Model Parameter Estimation] Given a model class $\left\{
  ...
  \end{equation}
  to characterize the source \ToDo{Mistake to use word source?} $Y$.
...
\item[Simulation] Given a model $P_{*\given \theta}$, one can
  ...
  is a hundred year flood?''.  \ToDo{Diane didn't know what a 100
    year flood is}  We often find that models that we fit
...
\ToDo{? Attach footnote 1 when you first use big X?}
...
\emph{Markov process} \index{Markov process} if \ToDo{ t is integer}
...
\end{align}
\ToDo{Space before switching to specific u, v, etc states?}
...
\ToDo{T is both the map and the length?  Diane wants to say again
...
produced a sequence of integer valued observations \ToDo{use python
  sequence notation} $y_1^{\syntheticNLorenzBig}$ by binning $x_1$
...
\ToDo{Plot goes beyond 39, but text says y[0:40]}
\begin{figure}[htbp]
  \centering{\plotsize%
