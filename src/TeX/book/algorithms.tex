...
write
\begin{equation*}
  P(\ts{y}{0}{T}) = P(\ti{y}{1}) \prod_{t=2}^T P(\ti{y}{t}\given \ts{y}{0}{t}).
\end{equation*}
...
\begin{equation}
 ...
 \right) + \sum_{t=2}^T \log\left( P(\ti{y}{t}\given \ts{y}{0}{t})\right).
\end{equation}
...
\begin{align*}
  \gamma(1) &= P_{\ti{Y}{1}}(\ti{y}{1}) =
              \sum_{s}P_{\ti{S}{1}}(s)P_{\ti{Y}{t}\given \ti{S}{t}}(\ti{y}{1}\given s)
  \\
  \alpha(s,1) &= P_{\ti{S}{1}\given \ti{Y}{1}}(s\given \ti{y}{1}) =
  \frac{P_{\ti{S}{1}}(s)P_{\ti{Y}{t}\given \ti{S}{t}}(\ti{y}{1}\given s)}
  {\gamma(1)}
~\forall s \in \states
\end{align*}
where the distributions $P_{\ti{S}{1}}$ and $P_{\ti{Y}{t}\given \ti{S}{t}}$
  ...
  \centering{\plotsize%
    ...
    \def\bestpred{%
      \begin{minipage}[t]{3.9in}
        ...
        one that maximizes\\%
        $\log\left(P(s\given s'(s,t)) \right) + \nu(s'(s,t),t-1)$.\\%
    ...
    \def\newnu{%
      \begin{minipage}[t]{2.7in}
        ...
        \ie, $ \nu(s,t) = \log\left(P(\ti{y}{t}\given s) \right) $\\
        \qquad$+ \log\left(P(s\given s'(s,t)) \right)$\\
        \qquad$+ \nu(s'(s,t),t-1)$.
      \end{minipage}}
    %%
...
\nomenclature[gb]{$\beta(s,t)$}{An intermediate quantity calculated in the
  ...
  \begin{align*}
    \beta(s,t) &= \frac{P_{\ts{Y}{t+1}{T}"\given \ti{S}{t}}
...
%    \begin{tabular*}{0.98\textwidth}[H]{|l|r|l|}
    \begin{tabular}[H]{|c|c|c|}
      ...
      & $P_{\ti{S}{1}\given \ti{\parameters}{n+1}} \left(s\given \ti{\parameters}{n+1} \right)$
      & $ w(s,1) $ \\[1.5ex]
...
\begin{equation*}
  ...
  \left(\ti{s}{1} \right) \cdot \prod_{t=2}^T P_{\ti{S}{2}\given \ti{S}{1}}
  \left(\ti{s}{t}\given \ti{s}{t-1} \right)
  \cdot \prod_{t=1}^T  P_{\ti{Y}{1}\given \ti{S}{1}}
  \left(\ti{y}{t}\given \ti{s}{t} \right),
\end{equation*}
or equivalently,
\begin{align*}
  \log P\left(\ts{y}{0}{T}, \ts{s}{0}{T} \right) &= \log P_{\ti{S}{1}}
  \left(\ti{s}{1} \right) + \sum_{t=2}^T \log P_{\ti{S}{2}\given \ti{S}{1}}
  \left(\ti{s}{t}\given \ti{s}{t-1} \right)\\
  &\quad + \sum_{t=1}^T \log  P_{\ti{Y}{1}\given \ti{S}{1}}
  \left(\ti{y}{t}\given \ti{s}{t} \right).
\end{align*}
...
%
  ...
  \left(\ts{s}{0}{T}\given \ts{y}{0}{T},\parameters \right) \log P_{\ti{S}{1}\given \parameters'}
  \left(\ti{s}{1}\given \parameters' \right) \right)\\
%
  \label{eq:Qtransition}
  Q_{\text{transition}} (\parameters',\parameters) &\equiv
%
  \sum_{\ts{s}{0}{T}\in\states^T} \left( P
  \left(\ts{s}{0}{T}\given \ts{y}{0}{T},\parameters \right) \sum_{t=2}^T \log
  P_{\ti{S}{2}\given \ti{S}{1},\parameters'}
...
%
  ...
  P_{\ti{Y}{1}\given \ti{S}{1},\parameters'}
...
\begin{align}
  L_{\text{initial}}(i) & \equiv \log P_{\ti{S}{1}\given \parameters'}
  \left(s\given \parameters' \right) \\
  L_{\text{transition}}(i,j) &\equiv \log P_{\ti{S}{2} \given 
    \ti{S}{1},\parameters'}({\tilde s}\given s,\parameters') \\
  \label{eq:Loutput}
  L_{\text{observation}}(y,i) & \equiv \log
  P_{\ti{Y}{1}\given \ti{S}{1},\parameters'} \left(y\given s,\parameters' \right).
\end{align}
...
\begin{align}
  ...
  \left(\ts{s}{0}{T}\given \ts{y}{0}{T},\parameters \right)\\
  &= \sum_s L_{\text{initial}}(s) P_{\ti{S}{1}\given \ts{Y}{0}{T},\parameters}
...
\begin{equation*}
  ...
  P_{\ti{S}{1}\given \hat \parameters} \left(s\given \hat \parameters \right) = 1.
\end{equation*}
...
\begin{equation}
  \label{eq:InitialRe}
  P_{\ti{S}{1}\given \ti{\parameters}{n+1}} \left(s\given \ti{\parameters}{n+1} \right) = w(s,1).
\end{equation}
...
\begin{align}
  ...
  \sum_{\ts{s}{0}{T}\in\states^T} P
  \left(\ts{s}{0}{T}\given \ts{y}{0}{T},\parameters \right) \sum_{t=2}^T
...
\begin{align}
  ...
  \sum_{\ts{s}{0}{T}\in\states^T} P
  \left(\ts{s}{0}{T}\given \ts{y}{0}{T},\parameters \right) \sum_{t=1}^T
...
\item Modify the reestimated initial state probabilities using
  \begin{equation*}
    P_{\ti{S}{1}\given \ti{\parameters}{m+1}} \left(s\given \ti{\parameters}{m+1}
...
\begin{equation}
  \label{eq:GaussMix}
  P\left(\ts{y}{0}{T}\given \parameters \right) = \prod_{t=1}^T
