...
\item[$\bm{\parameters}$] The entire collection of parameters.  For
  ...
  \nomenclature[gtheta]{$\bm{\parameters}$}{The entire collection of
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
%%% #1 = time #2 = STATE #3 = time + 1 #4 = state
...
%%%
%%% #1 = STATE #2 = state #3 = time
\newcommand{\StateForecast}[3]{% state forecast P(S[t]|y[0:t])
  \colorbox{green}{%
...
%
\nomenclature[galpha]{$\alpha(t,s)$}{The conditional probability that at time
  $t$ the system is in state $s$ given the observations up to the
  present,
  \begin{equation*}
    \alpha(t,s) \equiv  P_{S(t) \given \ts{Y}{0}{t+1}}\left(s \given \ts{y}{0}{t+1}
    \right).
  \end{equation*}
  Also called the forward updated distribution in the Kalman filter
  literature.}
%
\nomenclature[ggamma]{$\gamma(t)$}{The probability of the present observation
...
%%%
%%% #1 = t #2 = t-1 #3 = OldState
\newcommand{\Backcast}[3]{% P(S[t] | \tilde S[t-1])
  \colorbox{pink}{%
...
\newcommand{\FuturePast}{%
  \colorbox{orange}{% P(y[t+1:T] |  y[0:t+1])
...
\begin{equation}
  ...
  {\alphax{t}{S}{t+1}{s}}.
\end{equation}
\nomenclature[gbeta]{$\beta(t,s)$}{An intermediate quantity calculated in the
  ...
  \begin{align*}
    \beta(t,s) &= \frac{P_{\ts{Y}{t+1}{T} \given \ti{S}{t}}
      ...
      \right)} {P \left(\ts{y}{t+1}{T} \given \ts{y}{0}{t+1} \right)}\\
    &= \frac{P_{\ti{S}{t} \given \ts{Y}{0}{T}} \left(s \given \ts{y}{0}{T} \right)}
    {\alpha(t,s)} = \frac{P_{\ti{S}{t} \given \ts{Y}{0}{T}}
      \left(s \given \ts{y}{0}{T} \right)} {P_{\ti{S}{t} \given \ts{Y}{0}{t+1}}
      \left(s \given \ts{y}{0}{t+1} \right)}.
  \end{align*}
...
The algorithm starts at the final time $T-1$ with $\beta$ set to
...
\end{align*}
Similarly, rewrite the denominator of Eqn.~\eqref{eq:back2} using
Bayes rule:
\begin{equation*}
...
\fbox{
  \begin{minipage}{0.90\textwidth}
    \begin{tabbing}
      ...
      \end{minipage}\\ \\%
      %%
      \nomenclature[ggamman]{$\bm{\alpha}_n,\bm{\beta}_n,\bm{\gamma}_n$}{The
        sets of values of $\alpha(t,s), \beta(t,s)$ and $\gamma(t)$
        based on the $n^{\text{th}}$ iteration of the Baum-Welch
        algorithm.}
      \begin{minipage}[b]{1.0\textwidth}
      ...
      \begin{minipage}[b]{1.0\textwidth}
        %
        $\bm{\beta}_n$ is a set of values calculated on the basis of
        the $n^{\text{th}}$ model $\oldmodel$ and the data
        $\ts{y}{0}{T}$.  See Eqns.~\eqref{eq:bw4} and
        \eqref{eq:backformula}.
         \begin{equation*}
...
Watanabe and Yamaguchi\cite{Watanabe04}.  \marginpar{Want 2025 recent}
%
...
It seems plausible that a trajectory of $\EMmap$ would converge to a
