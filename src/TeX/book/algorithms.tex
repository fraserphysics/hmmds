...
\ReviewerNote{Reviewer5 says to increase the intuition to equation ratio.
  Reviewer4 wants more illustrations and examples.  Shari says put
  Viterbi before Forward.  Patrick says put Forward first. }
...
\ReviewerNote{Reviewer 3 wants a little more informal verbal
  discussion of how the different algorithms work.  He specifically
  mentions the Viterbi algorithm.  Patrick wants ``more structure''.}
...
and a sequence of observations $\ts{y}{1}{T}$, iteration of the Baum
Welch algorithm \index{Baum Welch algorithm|textbf} produces a
...
\index{EM (expectation-maximization) algorithm}%
...
%%% fig:train
%%%
%%% FixMe: \addcontentsline{loa}{section}{Baum Welch model parameter optimization}
%%% FixMe: Is this a "figure" or an "algorithm"?
\begin{figure}[htbp]
  \begin{center}
    %\small%
  ...
  \begin{minipage}{0.90\textwidth}
    \begin{tabbing}
      ...
      \begin{minipage}[b]{1.0\textwidth}
        ...
        data $\ts{y}{1}{T}$.  See Eqns.~\eqref{eq:alpha} and
        \eqref{eq:forwardD}.
        \begin{equation*}
...
\index*{ergodic}, we also calculate ergodic initial state probability
estimates using
\begin{equation}
  \label{eq:Ps0ergodic}
  P_{\ti{S}{1}(\text{ergodic})} \left(s \right) =
  \frac{\sum_{t}w(i,t)}{\sum_{j,t}w(j,t)}
\end{equation}

\ReviewerNote{Reviewer4 says ``To make this point plausible and serious,
  it is very important to explain clearly what are the assumptions
  from a theoretical as well as a practical viewpoint.''  He wants
  this in the preface or introduction, but I think this may be the
  place to put it.  Similar subsections could discuss assumption
  testing.  Reviewer4 also wants to know, ``which types of dynamical
  systems can be treated appropriately with HMM and what are the data
  requirements?''  Patrick says to put assumptions in the
  introduction.  Nick says look up David Pollard wrt data
  requirements.  LeCam, M Law, L. Brown; Amari, f. divergences; LeCam
  and Yang, Asymptotic Methods in Statistics (Springer)

  Response to this comment: See \eqref{eq:assume_markov} and
  \eqref{eq:assume_output} for assumptions.  See
  \ref{sec:EstWholeDist} for data requirements}
...
systems that have complicated continuous state spaces.  For example we fit
models with roughly 10 states to electrocardiograms even though we
believe that it would require at least scores of continuous valued
parameters to specify the states of the physiological parameters that
affect the signal.  By fitting unrealistically simple models we reduce
the variance of the parameter estimates at the expense of having
parameters that are harder to interpret.  It is a version of the
classic \index{bias-variance tradeoff} bias-variance tradeoff.
...
%\ToDo{Ralf wants more discussion.  Is this just a hack?}
...
estimate.  However, like the MLE, the MAP is a point estimate.  It
does a poor job of characterizing the set of plausible parameters.
...
\ReviewerNote{Reviewer 2: Give citations for these other optimization
  schemes.}
...
discussion of Eqn.~\ref{eq:EMmax} should skip this section.
Dempster Laird and Rubin\cite{Dempster77} coined the term \emph{EM
  algorithm} in 1977.  More recent treatments include Redner and
...
The first two iterations of the EM algorithm produces the estimates
$\lambda =0.603$, $\mu_1 = -2.028$, and $\mu_2 = 1.885$.  Intermediate
calculations and density plots appear in Fig.~\ref{fig:GaussMix}.  The
estimates repeat without change to a precision of twelve decimal
digits after a total of 53 iterations, and the final estimate is
$\lambda = 0.674601277409$, $\mu_1 = -1.81907548168$, and $\mu_2 =
2.31697397996$.
...
\begin{figure}[htbp]
  \centering{\plotsize%
    ...
    %%
    $\qquad \ti{\parameters}{1}=(0.5, -1, 1) \qquad \ti{\parameters}{2}=(0.592, -2.024, 1.779) \qquad \ti{\parameters}{3}=(0.603, -2.028, 1.885)$\\[2.0ex]
    ...
    \begin{tabular}{r@{}c@{\cs}|*{10}{@{\cs}.@{\cs}|}}
      \st                  & \bch{t}                     & \bch{1}   & \bch{2}   & \bch{3}   & \bch{4}   & \bch{5}   & \bch{6}   & \bch{7}   & \bch{8}   & \bch{9}   & \bch{10} \\
                            \cline{2-12}
      \st                \v&\cs $\ti{y}{t}$              &   -1.02   &   -3.39   &   -3.38   &   2.45    &   -0.13   &   -1.66   &   3.09    &   2.01    &   0.03    &   -2.73  \\
                            \cline{2-12}
      \st                \v&\cs $\ti{w}{t}$              &    0.89   &    1.00   &    1.00   &   0.01    &    0.56   &    0.97   &   0.00    &   0.02    &   0.49    &    1.00  \\
\st$\ti{\parameters}{1}$ \v&\cs $\ti{w}{t}\ti{y}{t}$     &   -0.91   &   -3.39   &   -3.38   &   0.02    &   -0.07   &   -1.61   &   0.01    &   0.04    &   0.01    &   -2.71  \\
      \st                \v&\cs $(1-\ti{w}{t})\ti{y}{t}$ &   -0.12   &   -0.00   &   -0.00   &   2.44    &   -0.06   &   -0.06   &   3.08    &   1.98    &   0.01    &   -0.01  \\
                            \cline{2-12}
      \st                \v&\cs $\ti{w}{t}$              &    0.98   &    1.00   &    1.00   &   0.00    &    0.60   &    1.00   &   0.00    &   0.00    &   0.45    &    1.00  \\
\st$\ti{\parameters}{2}$ \v&\cs $\ti{w}{t}\ti{y}{t}$     &   -1.00   &   -3.39   &   -3.38   &   0.00    &   -0.08   &   -1.66   &   0.00    &   0.00    &   0.01    &   -2.73  \\
      \st                \v&\cs $(1-\ti{w}{t})\ti{y}{t}$ &   -0.02   &   -0.00   &   -0.00   &   2.45    &   -0.05   &   -0.00   &   3.09    &   2.01    &   0.01    &   -0.00  \\
                            \cline{2-12}
    \end{tabular}\\[2.0ex]
    \def\ptheta{$P_{\ti{\parameters}{1}}$}%
    \def\px{$P_{\parameters}$}%
    \def\Pthetax{$P_{\ti{\parameters}{3}}$}%
    \input{figs/gaussmixA.eps_t}\\[-2.5ex]
    \input{figs/gaussmix.eps_t}}
  \caption[\comment{fig:GaussMix }Two iterations of the EM algorithm.]%
  {Two iterations of the EM algorithm.  We use the algorithm to search
    for parameters of Eqn.~\eqref{eq:GaussMix} to maximize the
    likelihood of ten simulated observations that appear above in the
    row labeled $\ti{y}{t}$.  The next three lines report the
    weighting used for recalculating the conditional means, $\mu_1$
    and $\mu_2$ for $\ti{\parameters}{2}$ in the M step, and the
    following three lines report the same quantities for the
    calculation of $\ti{\parameters}{3}$.  The first three models
    appear in the last line of the table.  On the top axes, we
    ...
    $\ti{\parameters}{1} = (0.5, -1, 1)$, the starting distribution we
    chose for the EM algorithm.  On the bottom axes we plot the
...
\end{equation*}
is called the \emph{\index*{Gibbs Inequality}}\footnote{While most
...
\end{equation}
then the Gibbs inequality implies $H(\parameters' ||
\ti{\parameters}{n}) \geq H(\ti{\parameters}{n} ||
...
simple function.  If there is a bound $\bar P$ on $ P
\left(\y|\parameters \right) $ with
\begin{equation*}
...
\begin{equation}
  ...
  \begin{cases}
    L(\parameters) = L^*\\
    H(\parameters || \EMmap(\parameters)) = H(\parameters || \parameters) = H^* \\
...
\begin{figure}[htbp]
  ...
  {\plotsize%
    { \def\x#1{\mbox{\lower1.0ex\hbox{$#1$}}}%
      ...
      \input{figs/EM1.eps_t}}%
    {\def\xlab{$\parameters$}%
      \def\ylab{$\EMmap(\parameters)$}%
      \input{figs/EM2.eps_t}}}
...
\begin{CVSID}
%%%$Id: algorithms.tex,v 1.139 2007/06/07 05:12:02 karlheg Exp $
\end{CVSID}
