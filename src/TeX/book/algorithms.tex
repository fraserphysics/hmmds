...
--including all of the HMMs the we describe in this book-- for which
one can write algorithms that calculate a unique $\EMmap(\theta)$.
...
$\theta^* \equiv \lim_{n \rightarrow \infty} \ti{\parameters}{n}$
...
\begin{equation}
  \label{eq:theta_true}
  \theta_{\text{true}} \equiv \left(u_{\text{true}}, v_{\text{true}}
...
\begin{equation}
  ...
  \theta_{\text{initial}} \equiv \left(u_{\text{initial}}, v_{\text{initial}}
  \right) = \left(0.001, 0.01 \right)
\end{equation}
we ran fifty iterations of the Baum-Welch algorithm to calculate an
estimate of $\theta^*$.  During training we held all the model
...
$U \times V$, of the first eleven training iterations.  Notice that
the trajectory quickly lines up with the eigenvector with the
eigenvalue closest to 1 and then decays exponentially towards
$\theta^*$.  We believe that behavior is typical.  If one could
estimate the spectral decomposition of
$\left.  \frac{\partial \EMmap(\theta)}{\partial\theta}
\right|_{\theta^*}$ efficiently, one could speed up the convergence of
HMM training.  The auxiliary function $Q$ is easy to evaluate and
maximize and consequently the code for plotting its level sets runs
quickly.  Plotting the level sets of the likelihood takes much more
time.
...
\begin{figure}[htbp]
  ...
  {An illustration of the EM algorithm converging to a maximum
    likelihood estimate $\theta^*$.  The data $\ts{y}{0}{10,000}$ come
    from simulating the HMM described in
    \eqref{eq:EM_convergence_example} and Figure~\ref{fig:EMxfig}.
    The first eleven estimates of $\theta\equiv(u,v)$ in the training
    sequence appear as a blue trajectory in both plots.  In the upper
    plot level sets of the log-likelihood,
    $L(\theta') = \log \left( P(\ts{y}{0}{10,000} \given
      \theta'\right)$, and auxiliary function,
    $Q(\theta',\ti{\theta}{n})$ appear for every third value of $n$.
    Notice that every value of $\theta'$ except $\ti{\theta}{n}$ on
    the level set of $Q(\theta',\ti{\theta}{n})$ is inside of (and
