...
noise $\sigma_\eta = \toyHviewObservationNoise$, occasionally the forecast deviations are
...
\begin{figure}[htbp]
  ...
  {These plots illustrate dynamical stretching increasing the variance
    ...
    distribution at $t=\toyHviewTMaxVar$ (on the left) to the
...
We have chosen the parameters\footnote{ In making Fig.~\ref{fig:ToyH},
  ...
  and $0 < \tau_s \leq 0.5$.} specified in the caption of
...
estimate $\hat \lambda_1 = \toyBenettinLambdaOne$ (base $e$) that we obtain using the
...
\begin{figure}[htb]
  ...
  the Lorenz system.]%
  {The effect of noise on Lyapunov exponent calculation for the Lorenz
    system.  In the upper plot, the colored lines trace
...
without bound\footnote{If for example,
  ...
  \end{equation*}
  then $\hat \lambda_i = \lambda$, but
  $\lim_{\delta \rightarrow 0}\hat \lambda_{\text{aug},i} = \infty$. }
as $ \left| \ti{r_{i,i}}{n} \right|$ varies more with $n$.  In
Fig.~\ref{fig:benettin} we compare $\hat \lambda_{\text{aug},1}$ with
$\hat \lambda_{1}$ for the Lorenz
...
that the Benettin procedure produces a robust practical upper bound on
model performance.
...
$\ts{y}{1}{t}$ up to time $t$, only a small fraction of the states
...
rightmost point on the curve\footnote{While the address space limit of
  ...
  of this book to about 1,500,000, computaion time constrains the
...
\begin{figure}[htbp]
  ...
  {Entropy gap, $\hat \delta_{\mu||\theta}$ vs number of states in
    ...
    The lower line indicates an estimate of the entropy rate,
    $\hat h(F,\mu)) = \hat \lambda_1$, of the true process based on
    Lyapunov exponents estimated by the Benettin procedure.  The
    ...
    technology.  Each model was built using the same
    \toyLikeLorntrain~sample trajectory in the original state space,
    and the cross entropy estimates are based on a test sequence of
    \toyLikeLorntest~observations.}
  \label{fig:LikeLor}
\end{figure}
