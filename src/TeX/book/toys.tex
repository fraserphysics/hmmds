...
measurements has the following characteristics:
  ...
  \item[Time step] We obtain the map $F$ by numerically integrating
    ...
    absolute error tolerance of $10^{-7}$. \marginpar{FixMe: Really?}
...
The largest excursion of $\ti{\sigma_\gamma}{t}$ in \marginpar{FixMe:
  Really?  Put values on x axis.}
...
\marginpar{FixMe: Get values from code.  t axis values.}
\begin{figure}[htbp]
  \centering{\plotsize%
...
\end{figure}
%%%
\marginpar{FixMe: Values from code.  Label traces.}
\begin{figure}[htbp]
...
\marginpar{FixMe: Values from code}
\begin{figure}[htbp]
  \centering{\plotsize%
    \def\tsa{$t_s$}%
...
\begin{figure}[htb]
  ...
  {Lyapunov exponent calculation for the Lorenz system.  In the upper
    ...
    $\frac{1}{T} \sum_{t=1}^T \log\left( \left| \ti{r_{1,1}}{t}
      \right| \right)$ for three different initial conditions and the
    black traces the 5\% and 95\% limits on 1,000 separate runs.  The
    lower part is the same except that
    $\left| \ti{r_{1,1}}{t} \right|$ is augmented by a noise term with
