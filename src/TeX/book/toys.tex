...
\index{entropy} If a discrete random variable $U$ takes on the values
$u_0,u_1,\ldots,u_n$ with probabilities $P(u_0),P(u_1),\ldots,P(u_n)$,
then the \emph{entropy} of $U$ is \nomenclature[rHU]{$H(U)$}{The
  entropy of a discrete random variable $U$.}
\begin{equation}
  \label{eq:HDef}
  H(U) \equiv - \EV \log \left( P(U) \right) = -\sum_{k=0}^n P(u_k)
...
\item[Probability density] The probability density for the initial
  state $P(\ti{x}{0}\given \theta)$ is a uniform distribution on a cube in
  $X$ that has length $\Lic$ on each side. %
\nomenclature[rLic]{$\Lic$}{The length of cube edges with uniform
  probability density at the initial time.}
%
