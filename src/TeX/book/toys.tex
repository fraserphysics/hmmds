...
measurements has the following characteristics:
  ...
  \item[Quantization] The observations are quantized with a resolution
    $\Delta$.  We analyze quantized measurements rather than
...
Lorenz system and the tangent equations.  We use those approximations
...
in which dynamical stretching, \ie, the linear instability of $\left[
  DF(x)\right]$, occasionally limits predictability.  We chose the
...
time step which appears in bottom plot of the figure is low when
...
the map $[DF]$ that occurs in that interval; the dynamics maps the
smaller ellipse in the plot on the left into the larger ellipse in the
plot on the right.

\begin{figure}[htbp]
  \centering{\plotsize%
    \def\loglikelihood{$\log\left( P_{\gamma} \left(\ti{y}{t} \right)\right)$}%
    \def\dev{$\ti{\sigma_\gamma}{t}$}%
    \def\errorerrore{$\ti{y}{t} - \ti{\mu_\gamma}{t}$}%
    \def\yt{$\ti{y}{t}$}%
    \input{figs/ToyTS1.eps_t}}
...
\begin{figure}[htbp]
  \centering{\plotsize%
    \def\S{$P_a\text{ and } P_\alpha\text{ at }t=118$}%
    \input{figs/ToyStretch118.eps_t}%
    \def\S{$P_a\text{ and } P_\alpha\text{ at }t=119$}%
    \input{figs/ToyStretch119.eps_t}}
  \caption[\comment{fig:ToyStretch }Dynamical stretching increases state variance.]%
...
We have chosen the parameters\footnote{ In making Fig.~\ref{fig:ToyH},
  ...
  absolute integration tolerance of $10^{-7}$.  We then chose the
...
estimate $\hat \lambda_1 = 0.906$ (base $e$) that we obtain using the
...
\begin{figure}[htbp]
  \centering{\plotsize%
    ...
    \def\tsb{\mbox{\lower6.0ex\hbox{$t_s$}}}%
    \def\ailly{$-\hat h$}%
    \input{figs/ToyH.eps_t}}
...
of slope of the map.  Specifically, the slope of $F_2$ is 2 and
...
The \index*{Hausdorff dimension}\footnote{We sometimes refer to sets and
  characteristics of sets with non-integer dimensions as
  \emph{fractal}.} of the middle third Cantor set is $\delta =
\frac{\log(2)}{\log(3)}$, and that is the factor that is missing in
the formula connecting entropy and stretching.
\begin{align}
  h_{KS}(F_3,\mu_c) &= \log(2) \nonumber \\
...
\begin{figure}[htb]
  \centering{\plotsize%
    ...
    \input{figs/benettinA.eps_t}
    \def\t{$t$}%
    \input{figs/benettinAug0.eps_t}}
  \caption[\comment{fig:benettin }Lyapunov exponent calculation for the Lorenz system.]%
...
\ReviewerNote{Reviewer 2 asks, ``If ARHMMs are not very good, why bother?''}


\begin{CVSID}
%%%$Id: toys.tex,v 1.62 2007-06-07 05:12:02 karlheg Exp $
\end{CVSID}
%%%
%%% Local Variables:
%%% TeX-master: "main"
