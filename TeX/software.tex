...
After fetching the file \emph{webhmmds.tar.bz2}, you can type the
following in a terminal window to unpack the software and build this
document:
\begin{verbatim}
>tar -xjf webhmmds.tar.bz2
>cd webhmmds
>make TeX/software.pdf
\end{verbatim}
If rather than typing \texttt{make TeX/software.pdf} you simply typed
\texttt{make}, you would build a version of the book.  In either case
the build process would take several hours and it might fail if your
environment doesn't match mine.  If you have more than one processor
and more than four GB of RAM, typing \texttt{make -j10
  TeX/software.pdf} might reduce the build time to less than an hour.

If you want to see a particular figure built, for example the cover
\emph{fig/Statesintro.pdf}, try:
\begin{verbatim}
>tar -xjf webhmmds.tar.bz2
>cd webhmmds
>make fig/Statesintro.pdf
\end{verbatim}

\chapter{Preface}
\label{chap:preface}

This document describes the software I used to produce \emph{Hidden
  Markov models and dynamical systems}, hereinafter \emph{the book}.
When I started working on the book, I decided to structure it as a
software project using version control to help with collaboration keep
track of changes.  I used and gnu \emph{make} to control building the
figures and formatting the text.  I hoped that using the structure of
a software project would produce a single package that achieved the
following goals:
\begin{itemize}
...
Here is a list all of the directories in this the \emph{webhmmds}
project and some of the key files:
\begin{description}
\item[Makefile:] Controls the building of data files and figures and
  formatting \LaTeX.  Typing ``make TeX/software.pdf'' produces this
  document, and simply typing ``make'' produces a version of the book.
\item[TeX/] \LaTeX source files for the book and this document
\item[figs/] A directory to hold the derived *.pdf and *.pdf\_t
  figure files.  None of the figure files are included; they must be
  built using the software.
\item[code/] Programs for processing data and making figures
  \begin{description}
  \item[hmm/] Modules that contain subclasses of the basic HMM class.
    The basic HMM class is part of a separate project called
    \emph{chestnut}.  I include that part of \emph{chestnut} here for
    convenience.
  \item[plotscripts/] Scripts that read from the \emph{data} directory
    and write figures to the \emph{figs} directory
  \item[python/] Scripts that read from the \emph{data} directory do
    manipulations and write results to the \emph{data} directory
  \item[xfigs/] Drawings made using the tool \emph{xfig}
  \end{description}
\item[data/] Source and derived data files.  I only distribute derived
  files that require many hours of cpu time to produce.  I report the
  cpu time required for my 1 GHZ 64 bit Athlon PC.
  \begin{description}
  \item[Apnea/] The *.Rtimes files contain the time in centiseconds of
    estimates of the time that each heartbeat occurred.  I derived
    these files from PhysioNet's data using their WFDB software. 
  \item[LP5.DAT:] D.~Y.~Tang's laser measurements
  \item[PFsurvey:] Data for Fig.~6.10 in the book (takes more than two
    days to make)
  \item[Save\_Hview\_T\_100, Save\_Hview\_T\_118, and
    Save\_Hview\_T\_119:] Data saved from the old version of the GUI
    program \emph{code/python/Hview.py}.  The new version makes
    similar data but the pictures are not quite as nice
  \item[a03er\_seg:] Data for Figs.~6.1-6.3 derived from PhysioNet
    files using WFDB software
  \item[summary\_of\_training and event-2-answers:] Expert annotations
    for each minute of the ECG data.  I fetched these files from
    PhysioNet.
  \item[gutenberg-19355:] H.\,L.\, Menken's \emph{A book of Prefaces}
  \end{description}
\end{description}

In a number of ways, the project did not develop as I had imagined,
and I don't have a single package that achieves all of the goals.  I
wrote the \emph{webhmmds} package --of which document is a part-- after
submitting the final manuscript of the book to SIAM.  The code uses
the new \emph{numpy} and \emph{scipy} packages which did not exist
when I started writing the book.  Although it does not exactly
reproduce the results in the book, I believe that my new code is
easier to read than the old code and that it reflects a little bit
more experience with Python.  In short I recommend it.

I have the following four groups of files: 1.~A CVS repository called
\emph{hmmdsbook} that contains both the \LaTeX for the first draft of
the book and software that I used to process the data for the figures;
2.~An SVN repository called \emph{tex\_hmmds} that contains the
processed data from \emph{hmmdsbook}, revised \LaTeX, and revised
plotting scripts to make the manuscript that I submitted to SIAM;
3.~\emph{Fraser-SIAM.rar} a \emph{Roshal ARchive} of the result of
SIAM's editorial work on the manuscript (which I cannot distribute);
4.~An SVN repository called \emph{webhmmds} that contains new code for
processing the data and up to date \LaTeX source files.

Here are some notes on the goals and the code:
\begin{description}
\item[Keep a record of all techniques:] The \emph{hmmdsbook} files
  were useful for producing the current \emph{webhmmds} package.  For most
  programs I only had to upgrade from the obsolete Numeric and
  numarray packages to numpy and scipy.
\item[Allow readers to read, run, and modify the code:]  The new code
  runs, and I think it would be pretty easy to read or modify.
\item[Allow readers to verify results:] Unfortunately the results of
  the new code do not match the results in the book.  In part this is
  due to the fact that the restructured code does not make the same
  sequence of calls to \emph{random} as the old code.  It also
  reflects less time spent tweaking parameters.  While suspicious readers
  may examine the old \emph{hmmdsbook} package, they will find it
  harder to read and more difficult to run than the newer code.
\item[Make formatting techniques available to other authors:] Karl
  Hegbloom helped me work with the SIAM macros and the required
  Mathtime and Mathtime Plus fonts.  While I don't think we learned
  anything that generalizes, readers may examine the \emph{tex\_hmmds}
  package.
\end{description}

The remainder of this document contains a version of each figure in
the book.  While the book explains the mathematics behind the
algorithms, this document provides software implementation details.
I've copied the Chapter and Section organization as the book to make
it easy to move between the book and this document.

\section*{Execution time}

I have built the book on the following PCs
\begin{description}
\item[Eight core 2.8GHz 64 bit Intel Xeon] On this machine which had
  8GB RAM and ran Ubuntu 8.04, issuing ``time make -j8'' produced the
  entire book in 74 minutes using five hours and one minute of
  individual cpu time.
\end{description}
In the following sections, I report the time required by a single
cpu for my 1 GHZ 64 bit Athlon PC.
...
\section*{To Do}
\begin{description}
\item[Write Fast Code] Write new versions of algorithms\_*.py that are
  in \textit{core} now.  One new version to accept sparse matrices and
  another written in C.
\item[Laser Model] Find a stable period five orbit and get better
  versions of Figs.~\ref{fig:LaserLogLike}, \ref{fig:LaserStates} and
  \ref{fig:LaserForecast}.  Here is an approach:
  \begin{itemize}
  \item Find  range of radii from one of the two unstable fixed points
    from which orbit takes 5 oscillations on other side and then
    returns original side
  \item Calculate map $M:X_2\otimes X_3\mapsto X_2\otimes X_3$ at
    $X_1=0$ in neighborhood of first crossing of $X_1=0$ plane
  \item Minimize the distance between the first and second crossing
  \item Calculate the stability of the fixed point
  \end{itemize}

\item[Cover Figure] Find a model that produces some dots for every
  state in Fig.~\ref{fig:Statesintro}.  (This is not a high priority.)
\item[Extend Fig.~\ref{fig:LikeLor}] Write code that makes more
  efficient use of memory and extend the plot to $10^8$.  (This is not
  a high priority.)
\item[Update Makefile] Since Damian Eads is dropping the Chestnut
  project, I have incorporated the code into this project.  I should
  modify the Makefile so that appropriate targets depend on the newly
  included code.  (This is not a high priority.)
\end{description}
...
Following Karl Hegbloom, I define a utility named run\_gnuplot in the
file Makefile.  The utility operates on a list of arguments
consisting of a file containing gnuplot commands, data files, and the
names of figure files to be written.  In Makefile, I use run\_gnuplot
every time that I need the gnuplot utility for making a figure.
Henceforth, I when I write ``the script foo.gpt makes foobar.pdf from
data/foo'', I mean that I use the Makefile utility run\_gnuplot to
call the utility gnuplot with arguments that make the files
figs/foo.pdf and figs.foo.pdf\_t from the information in the file
data/foo.  The file figs/foo.pdf\_t contains \LaTeX commands for the
text in the figure and the file figs/foo.pdf contains the rest of the
figure.  In the \LaTeX source, writing
\texttt{$\backslash$input\{figs/foo.pdf\_t\}} invokes the figure.  An
advantage of the method is that you can define new \LaTeX commands in
the source and use them in the figure.  When I started rewriting the
software to make it easier to read, I hoped to avoid gnuplot and
run\_gnuplot because the commands are hard to read.  I found that I
needed them for plotting response surfaces like
Fig.~\ref{fig:LaserLogLike}.  Once I decided to use them for response
surfaces, I decided to continue use them for other plots because I
already had the scripts and because the resulting plots look good.

The program Laser\_data.py also makes data/LaserLogLike for
Fig.~\ref{fig:LaserLogLike}.  The program takes about 33 minutes to
maximize the likelihood of parameters for an extended Kalman filter.
The script LaserLogLike.gpt makes LaserLogLike.pdf from
data/LaserLogLike.
\begin{figure}[htbp]
  \centering{\plotsize%
...
I used the \emph{gsl}, gnu scientific library, to integrate the Lorenz
system and make the data for
Figs.~\ref{fig:TSintro}-\ref{fig:Statesintro}.  Although I used SWIG
to connect the gsl to python for the book, I use scipy.weave for the
newer software now (see code/python/lorenz.py).  It seems simpler.  To
translate data into a Fig.~\ref{fig:TSintro}, I called pylab from the
script code/plotscripts/TSintro.py.  I have not found pylab to be much
easier to use than gnuplot.
\begin{figure}[htbp]
  \centering{
%    \resizebox{\textwidth}{!}{\includegraphics{figs/TSintro.pdf}}
  ...
  \label{fig:TSintro} 
\end{figure}
I use gnuplot to make STSintro from data/states for Fig.~\ref{fig:STSintro}.
 \begin{figure}[htbp]
   \centering{\plotsize%
...
Figure~\ref{fig:Statesintro} is picture on the cover.  My son Adrian
wrote stateplot.py which makes the figure from data in the files
data/state0 -- data/state11 and data/lorenz.xyz.  To create the state
data files I used the HMM component of the \emph{chestnut} package
(see http://mloss.org/software/view/153/).  Training the model,
m12s.4y and decoding the states takes five minutes
...
write the core routines in C which will reduce the time by an order of
magnitude.
\begin{table}[htb]
...
commands defined in the file software.tex.
...
It takes three hours and 36 minutes for TrainChar.py to make the data
for Fig.~\ref{fig:TrainChar}.  I use gnuplot with TrainChar.gpt to
make TrainChar.pdf from data/TrainChar.
\begin{figure}
  \centering{\plotsize%
    \def\llps{$\frac{\log \left( P \left(\ts{y}{1}{T}|\ti{\parameters}{n} \right)\right)}{T}$}%
%    \input{figs/TrainChar.pdf_t}
  }
...
I use gnuplot to make each component of Fig.~\ref{fig:GaussMix}.  The
script gaussmix.gpt needs the file data/em\_yt to make gaussmix.pdf.
Rather than have em\_yt in the distribution, I should modify
code/python/em.py to write the file.
\begin{figure}[htbp]
  \centering{\plotsize%
    %% 
    %% Data generated by code/python/em.py
    %% 
    %% 
    %% 0 0.592235464363 -2.02434657272 1.77965344905 \\%
    %% 
    %% 
    %% 1 0.602707378184 -2.02772188643 1.88504047255 \\%
    %%
    \def\cs{\hspace{0.10em}}%
    \def\st{\rule{0pt}{2.25ex}}%
    \def\v{\vline}%
    %% '.' is decimal aligned, see "hmmdsbook.cls", search "dcolumn" or "columntype".
    {\scriptsize$\qquad \ti{\parameters}{1}=(0.5, -1, 1) \qquad
      \ti{\parameters}{2}=(0.592, -2.024, 1.779) \qquad
      \ti{\parameters}{3}=(0.603, -2.028, 1.885)$}\\[2.0ex]
    \def\ptheta{$P_{\ti{\parameters}{1}}$}%
    \def\px{$P_{\parameters}$}%
    \def\Pthetax{$P_{\ti{\parameters}{3}}$}%
%    \input{figs/gaussmixA.pdf_t}\\[-2.5ex]
%    \input{figs/gaussmix.pdf_t}
  }
...
The scripts EM1.gpt and EM2.gpt make the components of
Fig.~\ref{fig:EM} without needing any external data.
\begin{figure}[htbp]
  \centering
  {\plotsize%
    { \def\x#1{\mbox{\lower1.0ex\hbox{$#1$}}}%
      \def\xlab{\mbox{\lower4.0ex\hbox{$\parameters$}}}%
      \def\y#1{\mbox{\kern-0.5em\lower1.0ex\hbox{$#1$}}}%
      \def\ylab{\mbox{\kern-0.5em\lower1.5ex\hbox{$\parametersPrime$}}}%
      \def\zlab{$Q(\parametersPrime,\parameters)$}%
%      \input{figs/EM1.pdf_t}
    }%
    {\def\xlab{$\parameters$}%
      \def\ylab{$\EMmap(\parameters)$}%
%      \input{figs/EM2.pdf_t}
    }}
...
Laser\_plots.py uses pylab to make Fig.~\ref{fig:LaserHist} directly
from Tang's file data/LP5.DAT.
\begin{figure}[htbp]
  \centering{
...
The data for the time series plots within
Fig.~\ref{fig:ScalarGaussian} get made by ScalarGaussian.py and they
are plotted using pylab by SGO.py.  The three state transition
drawings all come from the same xfig file ScalarGaussian.fig.  Each
time software.tex invokes the drawing, it redefines the commands used
for the labels.
%%%
%%% fig:ScalarGaussian
%%%
...
\label{sec:related}
It takes 11 minutes for VStatePic.py to make the state data for
Fig.~\ref{fig:VARGstates} from data/lorenz.xyz.
\begin{figure}[p]
  \centering{\plotsize%
...
Save\_Hview\_T\_100, Save\_Hview\_T\_118, and Save\_Hview\_T\_119.
Which I made using the old version of GUI program Hview.py.  You can
use the new version to make similar files and figures, but I like the
way the old ones look.  The script ToyA.py makes the figures from the
data.
\begin{figure}[htbp]
  \centering{
...
It takes an hour and 28 minutes for Hsurvey.py to make data/Hsurvey
and data/HtauS.  I use gnuplot with Hsurvey.gpt to make
Fig.~\ref{fig:ToyH} from those data files.
\begin{figure}[htbp]
  \centering{\plotsize%
    \def\tsa{$\Tsamp$}%
...
Figure~\ref{fig:benettin} uses the file data/benettin which takes
eleven minutes for LyapPlot.py to make.  I use gnuplot and
LyapPlot.gpt to translate the data into benettin.pdf.
three xfig drawings.
\begin{figure}[htb]
  \centering{\plotsize%
    \def\NFp{95\%}%
...
It takes three hours and 55 minutes for Sparse\_hmm\_lor.py to make
data/LikeLor.  I use gnuplot and LikeLor.gpt to make
Fig.~\ref{fig:LikeLor} from the data.  Note: To make this figure you
need to have at least two gigabytes of RAM.
\begin{figure}[htbp]
  \centering{\plotsize%
...
The script apnea\_ts\_plots.py uses pylab to make
Figs.~\ref{fig:a03erA} - \ref{fig:ApneaNLD}.  While the heart rate data for
these figures comes from *.lphr files that the script rr2hr.py
creates from *.Rtimes files, I derived the rest of the data directly
from PhysioNet files using the PhysioNet's WFDB software.
...
The script apnea\_sgram.py uses pylab to make Fig.~\ref{fig:sgram} from
a11.Rtimes.  The pylab function specgram makes the spectrogram.
\begin{figure}
  \centering{
...
The script rr2hr.py takes about a minute to process the data in the
data/Apnea/*.Rtimes files to make all of the data/Apnea/*.lphr files.

\subsection{Extracting Respiration Information}
\label{sec:RESP}

The script respire.py uses the data in the data/Apnea/*Rtimes files to
make all of the data/Apnea/*.resp files and the
files mean.resp, AA.resp, AN.resp, and C.resp for Fig.~\ref{fig:LDA}.
It takes 34 minutes and 1.8 GB of RAM.
\begin{figure}
  \centering{
...
Figure~\ref{fig:class1} is not exactly like the figure in the book.
The results of the new software are enough different from the results
of the old software to concern me.  I will put more effort into
understanding the difference.  The prerequisites for making
Fig.~\ref{fig:class1} include all of the *.resp and *.lphr files and
the HMM model files mod\_A, mod\_C, mod\_L, mod\_M, and mod\_H.  The
script DoubleClassify.py with the flag --Single makes data files
pass1A, pass1B, pass1C, and pass1X that class1.py uses to create the
figure.
\begin{figure}[tbhp]
  \centering{
...
\label{sec:tune}
%
It takes more than two days to make the data for
Fig.~\ref{fig:PFsurvey}.  To let readers avoid that delay, I've
included the data in the source package.  If you want to build the
data, simply remove data/PFsurveyH, make data/PFsurveyH and copy it to
data/PFsurvey.
\begin{figure}
  \centering{\plotsize%
    \def\pow{Pow}%
