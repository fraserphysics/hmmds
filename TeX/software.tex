...
\begin{equation*}
  \nu(s|c_1^t) \equiv P(y_1^t|s(t) = s, c_1^t)
\end{equation*}
for every possible value $s$ of the ending state $s(t)$.  I define
\emph{complete domination} of one class history $\tilde c_1^t$ by a
second $\hat c_1^t$ as
\begin{equation*}
  \nu(s|\hat c_1^t) \geq \nu(s|\tilde c_1^t),~ \forall s,
\end{equation*}
which I denote $\nu(s|\hat c_1^t) \preceq \nu(s| \tilde c_1^t)$.
Because a completely dominated class history will never be required as
a part of a best class sequence, one can safely discard them as one
makes a forward pass through a data set.  However, to ensure optimally
one must retain and propagate every partial history that is not
completely dominated by a another retained partial history.
